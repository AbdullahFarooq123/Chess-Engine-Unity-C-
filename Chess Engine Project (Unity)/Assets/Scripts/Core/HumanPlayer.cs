...
namespace Chess.Game {
	public class HumanPlayer : Player {
			...
			if (boardUI.TryGetSquareUnderMouse (mousePos, out targetSquare)) {
				if (targetSquare.Equals (selectedPieceSquare)) {
					...
					if (Piece.IsColour (board.Square[targetIndex], board.ColourToMove) && board.Square[targetIndex] != 0) {
						...
						//System.Threading.Tasks.Task.Factory.StartNew(()=>TryMakeMove (selectedPieceSquare, targetSquare),System.Threading.Tasks.TaskCreationOptions.LongRunning);
					}
				}
			} else {
				CancelPieceSelection ();
		...
		void TryMakeMove (Coord startSquare, Coord targetSquare) {
			...
			int promotionType = Move.Flag.PromoteToQueen;
			MoveGenerator moveGenerator = new MoveGenerator();
			if (Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.RightAlt)) {
				if (Input.GetKey(KeyCode.Q))
					promotionType = Move.Flag.PromoteToQueen;
				else if (Input.GetKey(KeyCode.K))
					promotionType = Move.Flag.PromoteToKnight;
				else if (Input.GetKey(KeyCode.R))
					promotionType = Move.Flag.PromoteToRook;
				else if (Input.GetKey(KeyCode.B))
					promotionType = Move.Flag.PromoteToBishop;
			}
			...
			for (int i = 0; i < legalMoves.Count; i++) {
				...
				if (legalMove.StartSquare == startIndex && legalMove.TargetSquare == targetIndex) {
					if (legalMove.IsPromotion) {
                        if (legalMove.MoveFlag != promotionType)
                            continue;
                    }
					moveIsLegal = true;

...
namespace Chess.Game {
	public class BoardUI : MonoBehaviour {
		...
		public void SelectCheckSquare(Coord coord) {
			SetSquareColour(coord, boardTheme.lightSquares.check, boardTheme.darkSquares.check);
		}
		...
		public Vector3 PositionFromCoord (int file, int rank, float depth = 0) {
			if (whiteIsBottom) {
				return new Vector3 (-xPosition + file, -yPosition + rank, depth);
			}
			return new Vector3 (-xPosition + 7 - file, 7 - rank - yPosition, depth);
